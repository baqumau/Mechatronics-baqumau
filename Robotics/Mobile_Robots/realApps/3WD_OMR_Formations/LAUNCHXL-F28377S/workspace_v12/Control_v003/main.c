...
void main(void){
    ...
    measurements = (char *)malloc(bufferSize * sizeof(char));                       // Preallocate memory for measurement data.
    // Initializing SCIB:
...
void scic_init(){
    ...
    ScicRegs.SCICCR.all = 0x0007;                                                   // 1 stop bit, no loop-back,
                                                                                    // no parity, 8 char bits,
                                                                                    // asynchronous mode, idle-line protocol.
    ScicRegs.SCICTL1.all = 0x0002;                                                  // Enable TX, internal SCICLK,
                                                                                    // disable RX, RX ERR, SLEEP, TXWAKE.
    EALLOW;
...
void scic_msg(char *msg){
    int i = 0;                                                                      // Declaration of i as integer variable.
    while(msg[i] != '\0'){
        scib_xmit(msg[i]);                                                          // Transmit byte via SCIC peripheral.
        i++;
    }
}
//-----------------------------------------------------------------------------------------------------------------------
