...
// Function to delay for a specified number of microseconds:
void delay_us(unsigned long us){
    // Get the system clock frequency (e.g., 200 MHz):
    unsigned long systemFrequencyMHz = DEVICE_SYSCLK_FREQ/1000000;              // Divide by 1 million to get MHz.
    unsigned long cyclesNeeded = us*systemFrequencyMHz/125;                     // Calculate the required cycles.
    //--------------------------------------------
