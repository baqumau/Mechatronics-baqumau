/* This program was made for testing SCI communication peripherals of F28377S LaunchPad.
...
//-------------------------
char measurements_1[bufferSize];                                        // Vector to save measurements data from UART 1, and it is subsequently sent via UART 4 module.
char measurements_2[bufferSize];                                        // Vector to save measurements data from UART 1.
//---------------------------------------------------------------------------------------------------------------
...
Data_Struct UART4 = createDataStruct(bufferSize,1,3*Robots_Qty,16);
...
void __attribute__((interrupt)) UART1_RX_Handler(){
  ...
  if(UART1.flag[1]){
    ...
    for(i = 0; i < 3*Robots_Qty; i++){
      FMR.q_k[i] = roundToThreeDecimals(atof(UART1.MAT3.data[0][i]));   // Saving pose of OMRs formation along global reference frame.
    }
    ...
    snprintf(measurements_1,sizeof(measurements_1),":0,%1.3f,%1.3f,%1.3f,%1.3f,%1.3f,%1.3f;",FMR.q_k[0],FMR.q_k[1],FMR.q_k[2],FMR.q_k[3],FMR.q_k[4],FMR.q_k[5]);
    Serial1.println(measurements_1);                                    // Write control signals through UART 4.
  }
...
void __attribute__((interrupt)) UART4_RX_Handler(){
  ...
  if(UART4.flag[1]){
    ...
    for(i = 0; i < 3*Robots_Qty; i++){
      FMR.c_k[i] = roundToThreeDecimals(atof(UART4.MAT3.data[0][i]));   // Saving cluster space pose of OMRs formation along global reference frame.
    }
    ...
    snprintf(measurements_2,sizeof(measurements_2),"%1.3f,%1.3f,%1.3f,%1.3f,%1.3f,%1.3f;",FMR.c_k[0],FMR.c_k[1],FMR.c_k[2],FMR.c_k[3],FMR.c_k[4],FMR.c_k[5]);
    Serial.println(measurements_2);                                     // Write measurements via UART 1.
  }
