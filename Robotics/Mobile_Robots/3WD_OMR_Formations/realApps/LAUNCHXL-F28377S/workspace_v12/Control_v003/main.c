...
void main(void){
    ...
    // Creating data structure for a SMC technique in the cluster space (type 01):
    SMC = createSMC_Controller01(smc_Gains,unc_Values,dis_Values,sls_Gains,sls_dampFacts,upsilon_2,kapWeigths);
    ...
    if(CpuTimer1.InterruptCount <= final_iteration && flagcommand_0){
        ...
        switch(consys){
            ...
            case SMC_CS:{
                ...
                //-------------------------------------------------------------------------------------------------------
                // Computing SMC CS strategy (type 01):
                computeSMC_Controller01(SMC,REF.y_k,FMR.c_k,CSO.y_k,SLS.y_k,FMR.params);        // Update the current values for SMC CS strategy.
                //-------------------------------------------------------------------------------------------------------
                // Conditioning input torque control for OMRs formation:
                for(i = 0; i < 3; i++){
                  ...
                  errors_k[i] = FMR.c_k[i] - REF.y_k[i];
            ...
            case SMC_CSa:{
                ...
                compute_SlidingSurfaces(SLS,REF.y_k,FMR.c_k,CSO.y_k);                           // Update to current values for sliding surfaces.
                //-------------------------------------------------------------------------------------------------------
                // Computing SMC CS strategy (type 01 - CSa):
                computeSMC_Controller01a(SMC,REF.y_k,FMR.c_k,CSO.y_k,SLS.y_k,FMR.params);       // Update the current values for SMC CS strategy.
                //-------------------------------------------------------------------------------------------------------
                // Conditioning input torque control for OMRs formation:
                for(i = 0; i < 3; i++){
                    FMR.u_k[i] = clutch(saturation(SMC.y_k[i],-100.0f/ke_1,100.0f/ke_1),t_cl,sampleTime,CpuTimer1.InterruptCount);
        ...
        switch(mod_result){
            case 1:{
                ...
                for(i = 0; i < 3*Robots_Qty; i++){
                    ...
                    ftoa(roundToThreeDecimals(FMR.u_k[i]),FMR.us_k.data[i],3);                  // Saving same control signals of OMRs formation, but in string-format.
                    ...
                    ftoa(roundToThreeDecimals(FMR.w_k[i]),FMR.ws_k.data[i],3);                  // Saving same above angular velocities of OMRs formation, but in string-format.
                }
                break;
            }
            case 2:{
                ...
                ftoa(roundToThreeDecimals(REF.y_k[0]),var00,3);
                memset_fast(var01,0,bufferSize_3);
                ftoa(roundToThreeDecimals(REF.y_k[1]),var01,3);
                memset_fast(var02,0,bufferSize_3);
                ftoa(roundToThreeDecimals(REF.y_k[2]),var02,3);
                memset_fast(var03,0,bufferSize_3);
                ftoa(roundToThreeDecimals(REF.y_k[3]),var03,3);
                memset_fast(var04,0,bufferSize_3);
                ftoa(roundToThreeDecimals(REF.y_k[4]),var04,3);
                memset_fast(var05,0,bufferSize_3);
                ftoa(roundToThreeDecimals(REF.y_k[5]),var05,3);
                switch(consys){
                    case ADRC_RS:{
            ...
            case 3:{
                switch(consys){
                    ...
                    case SMC_CS:{
                        // Packing the relevant measurement variables of OMRs formation:
                        memset_fast(measurements,0,bufferSize_0);                               // Initialize measurements data chain.
                        snprintf(measurements,bufferSize_0,":0,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%lu;\n",FMR.cs_k.data[0],FMR.cs_k.data[1],FMR.cs_k.data[2],FMR.cs_k.data[3],FMR.cs_k.data[4],FMR.cs_k.data[5],var00,var01,var02,var03,var04,var05,var06,var07,var08,var09,var10,var11,FMR.us_k.data[0],FMR.us_k.data[1],FMR.us_k.data[2],FMR.us_k.data[3],FMR.us_k.data[4],FMR.us_k.data[5],(unsigned long)(CpuTimer1.InterruptCount));
                        break;
                    }
                }
                break;
        ...
        if(!flagcommand_0 && CpuTimer1.InterruptCount > 2*freq_hz_1){
            ...
            switch(reftype){
                case CIRCUMFERENCE_01:{
                    ...
                    float Cx_0 = 1500.0f;                                                       // [mm], initial reference's rotational centre along the workspace's x axis.
            ...
            switch(consys){
                ...
                case SMC_CS:{
                    ...
                    //-------------------------------------------------------------------------------------------------
                    // Initializing SMC strategy (type 01):
                    initSMC_Controller01(SMC,REF.Z_0,obs_z0,SLS.E_0,FMR.params);                // Initialize SMC strategy.
                    //-------------------------------------------------------------------------------------------------
                ...
                case SMC_CSa:{
                    ...
                    //-------------------------------------------------------------------------------------------------
                    // Initializing SMC strategy (type 01):
                    initSMC_Controller01a(SMC,REF.Z_0,obs_z0,SLS.E_0,FMR.params);               // Initialize SMC strategy.
                    //-------------------------------------------------------------------------------------------------
                    // Initializing input torque control as FMR.u_k, PWM control signals as FMR.v_k and omni-wheel angular velocities as FMR.w_k:
                    for(i = 0; i < 3*Robots_Qty; i++){
                        FMR.u_k[i] = 0.0f;                                                      // Initial torque control in the formation.
...
__interrupt void scic_rx_isr(void){
    ...
    if(SCIC.flag[1] && CpuTimer1.InterruptCount <= final_iteration && flagcommand_0){
        ...
        for(i = 0; i < 3; i++){
            ...
            memset_fast(SCIC.MAT3.data[SCIC.identifier][i],0,SCIC.MAT3.zSize);                  // Clear data in occupied SCIC char vector.
        }
        //---------------------------------------------------------------------------------------------------------------
...
void scic_msgXmit(char *msg){
    ...
    // Copying "msg" string to dedicated transmission buffer within SCIC structure:
    // if(SCIC.TX_charBuffer[SCIC.TX_bufferIndex] == '\0') memcpy_fast(SCIC.TX_charBuffer,msg,SCIC.TX_bufferSize);
    // else{
