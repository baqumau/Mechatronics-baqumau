...
Reference createReference(float sampleTime, enum Reference_Type reftype){
    ...
    switch(reftype){
        case MINGYUE_01:{
            ...
            REF.DIF_1 = createDifferentiator(Robots_Qty,sampleTime,1.0f,100.0f);            // Create first differentiator struct within reference builder REF structure.
            REF.DIF_2 = createDifferentiator(Robots_Qty,sampleTime,1.0f,100.0f);            // Create second differentiator struct within reference builder REF structure.
            break;
        }
    }
...
void initReference(Reference REF, enum Control_System consys, enum Reference_Type reftype, float z_0[]){
    ...
    switch(reftype){
        case MINGYUE_01:{
            float Xco_0[Robots_Qty], Xd1_0[2*Robots_Qty], Xd2_0[2*Robots_Qty];              // Vectors to place the initial conditions of associated converter COR and differentiator DIF_1 and DIF_2 within REF structure.
