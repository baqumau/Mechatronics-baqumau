...
Reference createReference(float sampleTime, Reference_Type reftype){
    ...
    //-----------------------------------------------
    REF.flag[0] = false;                                                                    // Set REF flag to false.
    return REF;
}
//---------------------------------------------------------------------------------------------------------------
...
void initReference(Reference REF, Control_System consys, Reference_Type reftype, float z_0[]){
    ...
    for(i = 0; i < s; i++){
        ...
        REF.Z_0[i] = z_0[i];                                                                // Save cluster initial conditions data for c(0) within REF structure.
        REF.Z_0[i+s] = z_0[i+s];                                                            // Save cluster initial conditions data for c(0) within REF structure.
        REF.Z_0[i+r] = z_0[i+r];                                                            // Save cluster initial conditions data for c(0) within REF structure.
        // Initial conditions for cluster space variables:
        REF.z1_k[i] = z_0[i];                                                               // Save initial cluster's pose z1(k = 0) for OMRs formation.
        REF.z1_kp1[i] = REF.Ts*z_0[i+s] + z_0[i];                                           // Save cluster's pose prediction z1(k + 1) within REF structure.
        REF.z2_k[i] = z_0[i+s];                                                             // Save initial cluster's velocity z2(k = 0) for OMRs formation.
        REF.z2_kp1[i] = REF.Ts*z_0[i+r] + z_0[i+s];                                         // Save cluster's velocity prediction z2(k + 1) within REF structure.
        REF.z3_k[i] = z_0[i+r];                                                             // Save initial cluster's acceleration z3(k = 0) for OMRs formation.
        REF.z3_kp1[i] = (1.0f/REF.Ts)*(REF.z2_kp1[i] - REF.z2_k[i]);                        // Simple differentiation operator to obtain the prediction z3(k + 1).
        ...
        case 2:{
            switch(consys){
                case SMC_CSa:{
                    ...
                    float OP1_0 = REF.Z_0[3]*SC2_0;                                         // Pre-compute operation OP1(k + 1).
                    float OP2_0 = REF.Z_0[3]*CC2_0;                                         // Pre-compute operation OP2(k + 1).
                    float OP3_0 = REF.Z_0[9]*SC2_0;                                         // Pre-compute operation OP3(k + 1).
                    float OP4_0 = REF.Z_0[9]*CC2_0;                                         // Pre-compute operation OP4(k + 1).
                    float OP5_0 = REF.Z_0[15]*SC2_0;                                        // Pre-compute operation OP5(k + 1).
                    float OP6_0 = REF.Z_0[15]*CC2_0;                                        // Pre-compute operation OP6(k + 1).
                    float OP7_0 = OP2_0*REF.Z_0[8];                                         // Pre-compute operation OP7(k + 1).
                    float OP8_0 = OP1_0*REF.Z_0[8];                                         // Pre-compute operation OP8(k + 1).
                    float OP9_0 = 2.0f*OP4_0*REF.Z_0[8];                                    // Pre-compute operation OP9(k + 1).
                    float OP10_0 = 2.0f*OP3_0*REF.Z_0[8];                                   // Pre-compute operation OP10(k + 1).
                    float OP11_0 = REF.Z_0[8]*REF.Z_0[8];                                   // Pre-compute operation OP11(k + 1).
                    float OP12_0 = (SC2_0*OP11_0 - CC2_0*REF.Z_0[14]);                      // Pre-compute operation OP12(k + 1).
                    float OP13_0 = (CC2_0*OP11_0 + SC2_0*REF.Z_0[14]);                      // Pre-compute operation OP13(k + 1).
                    float OP14_0 = REF.Z_0[3]*OP12_0;                                       // Pre-compute operation OP14(k + 1).
                    float OP15_0 = REF.Z_0[3]*OP13_0;                                       // Pre-compute operation OP15(k + 1).
                    float OP16_0 = OP3_0 + OP7_0;                                           // Pre-compute operation OP16(k + 1).
                    float OP17_0 = OP4_0 - OP8_0;                                           // Pre-compute operation OP17(k + 1).
                    float OP18_0 = OP5_0 + OP9_0 - OP14_0;                                  // Pre-compute operation OP18(k + 1).
                    float OP19_0 = OP6_0 - OP10_0 - OP15_0;                                 // Pre-compute operation OP19(k + 1).
                    //------------------------------
    ...
    for(i = 0; i < s; i++){
        REF.x1_k[i] = REF.X_0[i];                                                           // Save initial robots' pose x1(k = 0) for OMRs formation.
        REF.x1_kp1[i] = REF.Ts*REF.X_0[i+s] + REF.X_0[i];                                   // Save robots' pose prediction x1(k + 1) within REF structure.
        REF.x2_k[i] = REF.X_0[i+s];                                                         // Save initial robots' velocity x2(k = 0) for OMRs formation.
        REF.x2_kp1[i] = REF.Ts*REF.X_0[i+r] + REF.X_0[i+s];                                 // Save robots' velocity prediction x2(k + 1) within REF structure.
        REF.x3_k[i] = REF.X_0[i+r];                                                         // Save initial robots' acceleration x3(k = 0) for OMRs formation.
...
void computeCircumference01(Reference REF, Control_System consys, unsigned long iterations){
    ...
    if(REF.flag[0]){
        // Updating algorithm:
        for(i = 0; i < 3*Robots_Qty; i++){
            REF.x1_k[i] = REF.x1_kp1[i];                                                    // Update data for x1(k) within REF structure.
            REF.x2_k[i] = REF.x2_kp1[i];                                                    // Update data for x2(k) within REF structure.
            REF.x3_k[i] = REF.x3_kp1[i];                                                    // Update data for x3(k) within REF structure.
            REF.z1_k[i] = REF.z1_kp1[i];                                                    // Update data for z1(k) within REF structure.
            REF.z2_k[i] = REF.z2_kp1[i];                                                    // Update data for z2(k) within REF structure.
            REF.z3_k[i] = REF.z3_kp1[i];                                                    // Update data for z3(k) within REF structure.
        }
        ...
        switch(Robots_Qty){
            case 2:{
                //------------------------------Cluster Space--------------------------------
                REF.z3_kp1[0] = Ac*sinf(WTc);                                               // Compute d^2(xc)/dt^2.
                REF.z3_kp1[1] = Ac*cosf(WTc);                                               // Compute d^2(yc)/dt^2.
                REF.z3_kp1[2] = 0.0f;                                                       // Compute d^2(thc)/dt^2.
                REF.z3_kp1[3] = 0.0f;                                                       // Compute d^2(dc)/dt^2.
                REF.z3_kp1[4] = 0.0f;                                                       // Compute d^2(psi_1)/dt^2.
                REF.z3_kp1[5] = 0.0f;                                                       // Compute d^2(psi_2)/dt^2.
                Integration(REF.INT_2,REF.z3_kp1);                                          // Compute second integration to vector Z3(k + 1).
                Integration(REF.INT_1,REF.INT_2.y_k);                                       // Compute first integration to second integration output.
                for(i = 0; i < 3*Robots_Qty; i++){
                    REF.z1_kp1[i] = REF.INT_1.x2_kp1[i];                                    // Update data for vector Z1(k + 1) within REF structure.
                    REF.z2_kp1[i] = REF.INT_2.x2_kp1[i];                                    // Update data for vector Z2(k + 1) within REF structure.
                }
                //-------------------------------Robot Space---------------------------------
                float SC2_kp1 = sinf(REF.z1_kp1[2]);                                        // Pre-compute sin(thc).
                float CC2_kp1 = cosf(REF.z1_kp1[2]);                                        // Pre-compute cos(thc).
                float OP1_kp1 = REF.z1_kp1[3]*SC2_kp1;                                      // Pre-compute operation OP1(k + 1).
                float OP2_kp1 = REF.z1_kp1[3]*CC2_kp1;                                      // Pre-compute operation OP2(k + 1).
                float OP3_kp1 = REF.z2_kp1[3]*SC2_kp1;                                      // Pre-compute operation OP3(k + 1).
                float OP4_kp1 = REF.z2_kp1[3]*CC2_kp1;                                      // Pre-compute operation OP4(k + 1).
                float OP5_kp1 = REF.z3_kp1[2]*SC2_kp1;                                      // Pre-compute operation OP5(k + 1).
                float OP6_kp1 = REF.z3_kp1[2]*CC2_kp1;                                      // Pre-compute operation OP6(k + 1).
                float OP7_kp1 = REF.z3_kp1[3]*SC2_kp1;                                      // Pre-compute operation OP7(k + 1).
                float OP8_kp1 = REF.z3_kp1[3]*CC2_kp1;                                      // Pre-compute operation OP8(k + 1).
                float OP9_kp1 = REF.z2_kp1[2]*REF.z2_kp1[2]*SC2_kp1;                        // Pre-compute operation OP9(k + 1).
                float OP10_kp1 = REF.z2_kp1[2]*REF.z2_kp1[2]*CC2_kp1;                       // Pre-compute operation OP10(k + 1).
                float OP11_kp1 = OP1_kp1*REF.z2_kp1[2];                                     // Pre-compute operation OP11(k + 1).
                float OP12_kp1 = OP2_kp1*REF.z2_kp1[2];                                     // Pre-compute operation OP12(k + 1).
                float OP13_kp1 = 2.0f*REF.z2_kp1[2];                                        // Pre-compute operation OP13(k + 1).
                float OP14_kp1 = OP3_kp1*OP13_kp1;                                          // Pre-compute operation OP14(k + 1).
                float OP15_kp1 = OP4_kp1*OP13_kp1;                                          // Pre-compute operation OP15(k + 1).
                float OP16_kp1 = REF.z1_kp1[3]*(OP9_kp1 - OP6_kp1);                         // Pre-compute operation OP16(k + 1).
                float OP17_kp1 = REF.z1_kp1[3]*(OP10_kp1 + OP5_kp1);                        // Pre-compute operation OP17(k + 1).
                float OP18_kp1 = OP3_kp1 + OP12_kp1;                                        // Pre-compute operation OP18(k + 1).
                float OP19_kp1 = OP4_kp1 - OP11_kp1;                                        // Pre-compute operation OP19(k + 1).
                float OP20_kp1 = OP7_kp1 + OP15_kp1 - OP16_kp1;                             // Pre-compute operation OP20(k + 1).
                float OP21_kp1 = OP8_kp1 - OP14_kp1 - OP17_kp1;                             // Pre-compute operation OP21(k + 1).
                //------------------------------
                ...
                //------------------------------
                REF.x2_kp1[0] = REF.z2_kp1[0] + OP18_kp1;
                REF.x2_kp1[1] = REF.z2_kp1[1] + OP19_kp1;
                REF.x2_kp1[3] = REF.z2_kp1[0] - OP18_kp1;
                REF.x2_kp1[4] = REF.z2_kp1[1] - OP19_kp1;
                //------------------------------
                REF.x3_kp1[0] = REF.z3_kp1[0] + OP20_kp1;
                REF.x3_kp1[1] = REF.z3_kp1[1] + OP21_kp1;
                REF.x3_kp1[3] = REF.z3_kp1[0] - OP20_kp1;
                REF.x3_kp1[4] = REF.z3_kp1[1] - OP21_kp1;
                //------------------------------
                switch(consys){
                    case SMC_CSa:{
...
void computeInfinity01(Reference REF, Control_System consys, unsigned long iterations){
    ...
    if(REF.flag[0]){
        ...
        float ddd_xc_k = -Wc2*cosf(WTc);                                                    // Compute d^3(xc)/dt^3.
        float ddd_yc_k = -8.0f*Wc2*cosf(2.0f*WTc);                                          // Compute d^3(yc)/dt^3.
        switch(Robots_Qty){
            case 2:{
                //------------------------------Cluster Space--------------------------------
                REF.z3_kp1[0] = -Wc1*sinf(WTc) + REF.Z_0[12];                               // Compute d^2(xc)/dt^2.
                REF.z3_kp1[1] = -4.0f*Wc1*sinf(2.0f*WTc) + REF.Z_0[13];                     // Compute d^2(yc)/dt^2.
                REF.z3_kp1[3] = REF.Z_0[15];                                                // Compute d^2(dc)/dt^2.
                float Z3_1_k[3] = {REF.z3_kp1[0], REF.z3_kp1[1], REF.z3_kp1[3]};            // First vector of z3(k + 1) = d^2([xc,yc])/dt^2.
                Integration(REF.INT_3,Z3_1_k);                                              // Compute second integration to z3(k + 1).
                REF.z2_kp1[0] = REF.INT_3.x2_kp1[0];
                REF.z2_kp1[1] = REF.INT_3.x2_kp1[1];
                REF.z2_kp1[3] = REF.INT_3.x2_kp1[2];
                //------------------------------
                // Computing d^2(thc)/dt^2:
                float OP1_kp1 = REF.z2_kp1[0]*REF.z2_kp1[0];                                // Pre-compute operation OP1(k + 1).
                float OP2_kp1 = REF.z2_kp1[0]*REF.z2_kp1[1];                                // Pre-compute operation OP2(k + 1).
                float OP3_kp1 = REF.z2_kp1[1]*REF.z2_kp1[1];                                // Pre-compute operation OP3(k + 1).
                float OP4_kp1 = OP1_kp1 + OP3_kp1;                                          // Pre-compute operation OP4(k + 1).
                float OP5_kp1 = OP1_kp1 - OP3_kp1;                                          // Pre-compute operation OP5(k + 1).
                float OP6_kp1 = OP4_kp1*OP4_kp1;                                            // Pre-compute operation OP6(k + 1).
                float OP7_kp1 = 1.0f/OP6_kp1;                                               // Pre-compute operation OP7(k + 1).
                REF.z3_kp1[2] = -REF.z3_kp1[0]*(2.0f*REF.z3_kp1[0]*OP2_kp1 - REF.z3_kp1[1]*OP5_kp1)*OP7_kp1 + REF.z3_kp1[1]*(2.0f*REF.z3_kp1[1]*OP2_kp1 + REF.z3_kp1[0]*OP5_kp1)*OP7_kp1 + (ddd_xc_k*REF.z2_kp1[1] - ddd_yc_k*REF.z2_kp1[0])/OP4_kp1;
                float Z3_2_k[1] = {REF.z3_kp1[2]};                                          // Second vector of z3(k + 1) = d^2([xc,yc])/dt^2.
                Integration(REF.INT_2,Z3_2_k);                                              // Compute second integration to vector Z3(k + 1).
                ...
                Integration(REF.INT_1,Z2_1_k);                                              // Compute second integration to vector Z3(k + 1).
                ...
                float SC2_kp1 = sinf(REF.z1_kp1[2]);                                        // Pre-compute sin(thc).
                float CC2_kp1 = cosf(REF.z1_kp1[2]);                                        // Pre-compute cos(thc).
                OP1_kp1 = REF.z1_kp1[3]*SC2_kp1;                                            // Pre-compute operation OP1(k + 1).
                OP2_kp1 = REF.z1_kp1[3]*CC2_kp1;                                            // Pre-compute operation OP2(k + 1).
                OP3_kp1 = REF.z2_kp1[3]*SC2_kp1;                                            // Pre-compute operation OP3(k + 1).
                OP4_kp1 = REF.z2_kp1[3]*CC2_kp1;                                            // Pre-compute operation OP4(k + 1).
                OP5_kp1 = REF.z3_kp1[2]*SC2_kp1;                                            // Pre-compute operation OP5(k + 1).
                OP6_kp1 = REF.z3_kp1[2]*CC2_kp1;                                            // Pre-compute operation OP6(k + 1).
                OP7_kp1 = REF.z3_kp1[3]*SC2_kp1;                                            // Pre-compute operation OP7(k + 1).
                float OP8_kp1 = REF.z3_kp1[3]*CC2_kp1;                                      // Pre-compute operation OP8(k + 1).
                float OP9_kp1 = REF.z2_kp1[2]*REF.z2_kp1[2]*SC2_kp1;                        // Pre-compute operation OP9(k + 1).
                float OP10_kp1 = REF.z2_kp1[2]*REF.z2_kp1[2]*CC2_kp1;                       // Pre-compute operation OP10(k + 1).
                float OP11_kp1 = OP1_kp1*REF.z2_kp1[2];                                     // Pre-compute operation OP11(k + 1).
                ...
                switch(consys){
                    case SMC_CSa:{
                        REF.z3_kp1[4] = REF.x3_kp1[2];                                      // Compute d^2(psi_1)/dt^2.
                        REF.z3_kp1[5] = REF.x3_kp1[5];                                      // Compute d^2(psi_2)/dt^2.
                        REF.z2_kp1[4] = REF.x2_kp1[2];                                      // Compute d(psi_1)/dt.
                        REF.z2_kp1[5] = REF.x2_kp1[5];                                      // Compute d(psi_2)/dt.
                        REF.z1_kp1[4] = REF.x1_kp1[2];                                      // Compute psi_1.
                        REF.z1_kp1[5] = REF.x1_kp1[5];                                      // Compute psi_2.
                        break;
                    }
                    default:{
                        REF.z3_kp1[4] = REF.x3_kp1[2] - REF.z3_kp1[2];                      // Compute d^2(psi_1)/dt^2.
                        REF.z3_kp1[5] = REF.x3_kp1[5] - REF.z3_kp1[2];                      // Compute d^2(psi_2)/dt^2.
                        REF.z2_kp1[4] = REF.x2_kp1[2] - REF.z2_kp1[2];                      // Compute d(psi_1)/dt.
                        REF.z2_kp1[5] = REF.x2_kp1[5] - REF.z2_kp1[2];                      // Compute d(psi_2)/dt.
                        REF.z1_kp1[4] = REF.x1_kp1[2] - REF.z1_kp1[2];                      // Compute psi_1.
                        REF.z1_kp1[5] = REF.x1_kp1[5] - REF.z1_kp1[2];                      // Compute psi_2.
                        break;
                    }
                }
                break;
...
void computeInfinity02(Reference REF, Control_System consys, unsigned long iterations){
    ...
    if(REF.flag[0]){
        ...
        switch(Robots_Qty){
            case 2:{
                //------------------------------Cluster Space--------------------------------
                REF.z3_kp1[0] = -Wc1*sinf(WTc);                                             // Compute d^2(xc)/dt^2.
                REF.z3_kp1[1] = -4.0f*Wc1*sinf(2.0f*WTc);                                   // Compute d^2(yc)/dt^2.
                // Computing d^2(thc)/dt^2:
                float OP1_kp1 = REF.z2_kp1[0]*REF.z2_kp1[0];                                // Pre-compute operation OP1(k + 1).
                float OP2_kp1 = REF.z2_kp1[0]*REF.z2_kp1[1];                                // Pre-compute operation OP2(k + 1).
                float OP3_kp1 = REF.z2_kp1[1]*REF.z2_kp1[1];                                // Pre-compute operation OP3(k + 1).
                ...
                switch(consys){
                    ...
                    break;
                    default:
                    REF.z3_kp1[4] = -REF.z3_kp1[2];                                         // Compute d^2(psi_1)/dt^2.
                    REF.z3_kp1[5] = -REF.z3_kp1[2];                                         // Compute d^2(psi_2)/dt^2.
                    break;
                }
                Integration(REF.INT_2,REF.z3_kp1);                                          // Compute second integration to vector Z3(k + 1).
                Integration(REF.INT_1,REF.INT_2.y_k);                                       // Compute first integration to second integration output.
    ...
    if(REF.flag[0]){
        ...
        switch(Robots_Qty){
            case 2:{
                ...
                //------------------------------Cluster Space--------------------------------
                REF.z2_kp1[0] = 0.0f;                                                       // Compute d(xc)/dt.
                REF.z2_kp1[1] = 0.0f;                                                       // Compute d(yc)/dt.
                REF.z2_kp1[2] = 0.0f;                                                       // Compute d(thc)/dt.
                REF.z2_kp1[3] = 0.0f;                                                       // Compute d(dc)/dt.
                REF.z2_kp1[4] = d_ph1_k;                                                    // Compute d(psi_1)/dt.
                REF.z2_kp1[5] = d_ph2_k;                                                    // Compute d(psi_2)/dt.
                Integration(REF.INT_1,REF.z2_kp1);                                          // Compute only first integration to vector Z2(k + 1).
                for(i = 0; i < 3*Robots_Qty; i++){
                    REF.z1_kp1[i] = REF.INT_1.x2_kp1[i];                                    // Update data for vector Z1(k + 1) within REF structure.
                }
                //------------------------------
                REF.z3_kp1[0] = 0.0f;                                                       // Compute d^2(xc)/dt^2.
                REF.z3_kp1[1] = 0.0f;                                                       // Compute d^2(yc)/dt^2.
                REF.z3_kp1[2] = 0.0f;                                                       // Compute d^2(thc)/dt^2.
                REF.z3_kp1[3] = 0.0f;                                                       // Compute d^2(dc)/dt^2.
                REF.z3_kp1[4] = 0.0f;                                                       // Compute d^2(psi_1)/dt^2.
                REF.z3_kp1[5] = 0.0f;                                                       // Compute d^2(psi_2)/dt^2.
...
void computeIndepCircumferences01(Reference REF, Control_System consys, unsigned long iterations){
    int i;                                                                                  // Declaration of i as integer variable.
    // Executing builder algorithm for two independent ellipsoids trajectories:
    if(REF.flag[0]){
        // Updating algorithm:
        for(i = 0; i < 3*Robots_Qty; i++){
            REF.x1_k[i] = REF.x1_kp1[i];                                                    // Update data for x1(k) within REF structure.
        ...
        // Computing equations for generation of ellipsoidal reference profiles in the cluster space:
        switch(Robots_Qty){
            case 2:{
                float d_ph1_k = 0.2f*M_PI;                                                  // [rad/s], Desired angular velocity of robot 1.
                float d_ph2_k = -0.2f*M_PI;                                                 // [rad/s], Desired angular velocity of robot 2.
                ...
                float WTc = (float)(iterations)*REF.Ts*Wc;                                  // Rate variation of sinusoidal functions.
                float Wc_Wc = Wc*Wc;                                                        // Pre-compute Wc^2.
                //------------------------------Cluster Space--------------------------------
                REF.z2_kp1[0] = 0.0f;                                                       // Compute d(xc)/dt.
                REF.z2_kp1[1] = RYc*Wc*cosf(WTc);                                           // Compute d(yc)/dt.
                REF.z2_kp1[2] = 0.0f;                                                       // Compute d(thc)/dt.
                REF.z2_kp1[3] = RXc*Wc*sinf(WTc);                                           // Compute d(dc)/dt.
                REF.z2_kp1[4] = d_ph1_k;                                                    // Compute d(psi_1)/dt.
                REF.z2_kp1[5] = d_ph2_k;                                                    // Compute d(psi_2)/dt.
                Integration(REF.INT_1,REF.z2_kp1);                                          // Compute only first integration to vector Z2(k + 1).
                for(i = 0; i < 3*Robots_Qty; i++){
                ...
                //------------------------------
                REF.z3_kp1[0] = 0.0f;                                                       // Compute d^2(xc)/dt^2.
                REF.z3_kp1[1] = -RYc*Wc_Wc*sinf(WTc);                                       // Compute d^2(yc)/dt^2.
                REF.z3_kp1[2] = 0.0f;                                                       // Compute d^2(thc)/dt^2.
                REF.z3_kp1[3] = RXc*Wc_Wc*cosf(WTc);                                        // Compute d^2(dc)/dt^2.
                REF.z3_kp1[4] = 0.0f;                                                       // Compute d^2(psi_1)/dt^2.
                REF.z3_kp1[5] = 0.0f;                                                       // Compute d^2(psi_2)/dt^2.
                //-------------------------------Robot Space---------------------------------
                float SC2_kp1 = sinf(REF.z1_kp1[2]);                                        // Pre-compute sin(thc).
    ...
    if(REF.flag[0]){
        ...
        switch(Robots_Qty){
            case 2:{
                ...
                float Wc_Wc = Wc*Wc;                                                        // Pre-compute Wc^2.
                //------------------------------Cluster Space--------------------------------
                float OP1_kp1 = sinf(WTc);                                                  // Pre-compute operation OP1(k + 1).
                float OP2_kp1 = cosf(WTc);                                                  // Pre-compute operation OP2(k + 1).
                float OP3_kp1 = RYc*OP1_kp1;                                                // Pre-compute operation OP3(k + 1).
                float OP4_kp1 = RXc*OP2_kp1;                                                // Pre-compute operation OP4(k + 1).
                float OP5_kp1 = RXc*OP1_kp1;                                                // Pre-compute operation OP5(k + 1).
                float OP6_kp1 = RYc*OP2_kp1;                                                // Pre-compute operation OP6(k + 1).
                float OP7_kp1 = RXc - OP4_kp1;                                              // Pre-compute operation OP7(k + 1).
                float OP8_kp1 = dc_0 + OP7_kp1;                                             // Pre-compute operation OP8(k + 1).
                float OP9_kp1 = OP3_kp1*OP3_kp1;                                            // Pre-compute operation OP9(k + 1).
                float OP10_kp1 = OP5_kp1*OP5_kp1;                                           // Pre-compute operation OP10(k + 1).
                float OP11_kp1 = OP6_kp1*OP6_kp1;                                           // Pre-compute operation OP11(k + 1).
                float OP12_kp1 = OP8_kp1*OP8_kp1;                                           // Pre-compute operation OP12(k + 1).
                float OP13_kp1 = Wc*RXc;                                                    // Pre-compute operation OP13(k + 1).
                float OP14_kp1 = Wc*RYc;                                                    // Pre-compute operation OP14(k + 1).
                float OP15_kp1 = OP9_kp1 + OP12_kp1;                                        // Pre-compute operation OP15(k + 1).
                float OP16_kp1 = OP9_kp1 - OP12_kp1;                                        // Pre-compute operation OP16(k + 1).
                float OP17_kp1 = 1.0f/OP15_kp1;                                             // Pre-compute operation OP17(k + 1).
                float OP18_kp1 = OP17_kp1*OP17_kp1;                                         // Pre-compute operation OP18(k + 1).
                float OP19_kp1 = Wc_Wc*OP3_kp1;                                             // Pre-compute operation OP19(k + 1).
                float OP20_kp1 = 2.0f*OP19_kp1;                                             // Pre-compute operation OP20(k + 1).
                float OP21_kp1 = OP20_kp1*OP18_kp1;                                         // Pre-compute operation OP21(k + 1).
                float OP22_kp1 = 1.0f/sqrtf(OP15_kp1);                                      // Pre-compute operation OP22(k + 1).
                float OP23_kp1 = OP1_kp1*OP8_kp1*OP13_kp1;                                  // Pre-compute operation OP23(k + 1).
                float OP24_kp1 = OP2_kp1*OP3_kp1*OP14_kp1;                                  // Pre-compute operation OP24(k + 1).
                float OP25_kp1 = OP23_kp1 + OP24_kp1;                                       // Pre-compute operation OP25(k + 1).
                //------------------------------
                REF.z2_kp1[0] = 0.0f;                                                       // Compute d(xc)/dt.
                REF.z2_kp1[1] = 0.0f;                                                       // Compute d(yc)/dt.
                REF.z2_kp1[2] = OP14_kp1*(OP7_kp1 - dc_0*OP2_kp1)*OP17_kp1;                 // Compute d(thc)/dt.
                REF.z2_kp1[3] = OP25_kp1*OP22_kp1;                                          // Compute d(dc)/dt.
                switch(consys){
                    case SMC_CSa:
                    REF.z2_kp1[4] = d_ph1_k;                                                // Compute d(psi_1)/dt.
                    REF.z2_kp1[5] = d_ph2_k;                                                // Compute d(psi_2)/dt.
                    break;
                    default:
                    REF.z2_kp1[4] = d_ph1_k - REF.z2_kp1[2];                                // Compute d(psi_1)/dt.
                    REF.z2_kp1[5] = d_ph2_k - REF.z2_kp1[2];                                // Compute d(psi_2)/dt.
                    break;
                }
                Integration(REF.INT_1,REF.z2_kp1);                                          // Compute only first integration to vector Z2(k + 1).
                for(i = 0; i < 3*Robots_Qty; i++){
                ...
                OP13_kp1 = 2.0f*REF.z2_kp1[2];                                              // Pre-compute operation OP13(k + 1).
                OP14_kp1 = OP3_kp1*OP13_kp1;                                                // Pre-compute operation OP14(k + 1).
                OP15_kp1 = OP4_kp1*OP13_kp1;                                                // Pre-compute operation OP15(k + 1).
                OP16_kp1 = REF.z1_kp1[3]*(OP9_kp1 - OP6_kp1);                               // Pre-compute operation OP16(k + 1).
                OP17_kp1 = REF.z1_kp1[3]*(OP10_kp1 + OP5_kp1);                              // Pre-compute operation OP17(k + 1).
                OP18_kp1 = OP3_kp1 + OP12_kp1;                                              // Pre-compute operation OP18(k + 1).
                OP19_kp1 = OP4_kp1 - OP11_kp1;                                              // Pre-compute operation OP19(k + 1).
                OP20_kp1 = OP7_kp1 + OP15_kp1 - OP16_kp1;                                   // Pre-compute operation OP20(k + 1).
                OP21_kp1 = OP8_kp1 - OP14_kp1 - OP17_kp1;                                   // Pre-compute operation OP21(k + 1).
                //------------------------------
                REF.x1_kp1[0] = REF.z1_kp1[0] + OP1_kp1;
                REF.x1_kp1[1] = REF.z1_kp1[1] + OP2_kp1;
                REF.x1_kp1[3] = REF.z1_kp1[0] - OP1_kp1;
    ...
    if(REF.flag[0]){
        ...
        switch(Robots_Qty){
            case 2:{
                ...
                float OP1_kp1 = sinf(WTc);                                                  // Pre-compute operation OP1(k + 1).
                float OP2_kp1 = cosf(WTc);                                                  // Pre-compute operation OP2(k + 1).
                float OP3_kp1 = Wc*OP1_kp1;                                                 // Pre-compute operation OP3(k + 1).
                float OP4_kp1 = Wc*OP2_kp1;                                                 // Pre-compute operation OP4(k + 1).
                float OP5_kp1 = RXc_1*OP4_kp1;                                              // Pre-compute operation OP5(k + 1).
                float OP6_kp1 = RXc_2*OP3_kp1;                                              // Pre-compute operation OP6(k + 1).
                float OP7_kp1 = RYc_1*OP3_kp1;                                              // Pre-compute operation OP7(k + 1).
                float OP8_kp1 = RYc_2*OP4_kp1;                                              // Pre-compute operation OP8(k + 1).
                float OP9_kp1 = AXc + RXc_1*(1.0f + OP1_kp1) + RXc_2*(1.0f - OP2_kp1);      // Pre-compute operation OP9(k + 1) = a(WTc).
                float OP10_kp1 = RYc_1*OP2_kp1 - RYc_2*OP1_kp1;                             // Pre-compute operation OP10(k + 1) = b(WTc).
                float OP11_kp1 = OP5_kp1 + OP6_kp1;                                         // Pre-compute operation OP11(k + 1) = d[a(WTc)]/dt.
                float OP12_kp1 = -(OP7_kp1 + OP8_kp1);                                      // Pre-compute operation OP12(k + 1) = d[b(WTc)]/dt.
                float OP13_kp1 = OP9_kp1*OP9_kp1 + OP10_kp1*OP10_kp1;                       // Pre-compute operation OP13(k + 1) = a(WTc)^2 + b(WTc)^2.
                float OP14_kp1 = sqrtf(OP13_kp1);                                           // Pre-compute operation OP14(k + 1).
                float OP15_kp1 = RXc_1*Wc_Wc*OP1_kp1;                                       // Pre-compute operation OP15(k + 1).
                float OP16_kp1 = RXc_2*Wc_Wc*OP2_kp1;                                       // Pre-compute operation OP16(k + 1).
                float OP17_kp1 = RYc_1*Wc_Wc*OP2_kp1;                                       // Pre-compute operation OP17(k + 1).
                float OP18_kp1 = RYc_2*Wc_Wc*OP1_kp1;                                       // Pre-compute operation OP18(k + 1).
                float OP19_kp1 = OP16_kp1 - OP15_kp1;                                       // Pre-compute operation OP19(k + 1) = d^2[a(WTc)]/dt^2.
                float OP20_kp1 = OP18_kp1 - OP17_kp1;                                       // Pre-compute operation OP20(k + 1) = d^2[b(WTc)]/dt^2.
                float OP21_kp1 = OP10_kp1*OP11_kp1 - OP9_kp1*OP12_kp1;                      // Pre-compute operation OP21(k + 1) = b(WTc)*d[a(WTc)]/dt - a(WTc)*d[b(WTc)]/dt.
                float OP22_kp1 = OP9_kp1*OP11_kp1 + OP10_kp1*OP12_kp1;                      // Pre-compute operation OP22(k + 1) = a(WTc)*d[a(WTc)]/dt + b(WTc)*d[b(WTc)]/dt.
                float OP23_kp1 = 1.0f/OP13_kp1;                                             // Pre-compute operation OP23(k + 1).
                float OP24_kp1 = 1.0f/OP14_kp1;                                             // Pre-compute operation OP24(k + 1).
                //------------------------------
                REF.z2_kp1[0] = (OP5_kp1 - OP6_kp1)*0.5f;                                   // Compute d(xc)/dt.
                REF.z2_kp1[1] = (OP8_kp1 - OP7_kp1)*0.5f;                                   // Compute d(yc)/dt.
                REF.z2_kp1[2] = OP21_kp1*OP23_kp1;                                          // Compute d(thc)/dt.
                REF.z2_kp1[3] = 0.5f*OP22_kp1*OP24_kp1;                                     // Compute d(dc)/dt.
                switch(consys){
                    case SMC_CSa:
                    REF.z2_kp1[4] = d_ph1_k;                                                // Compute d(psi_1)/dt.
                    REF.z2_kp1[5] = d_ph2_k;                                                // Compute d(psi_2)/dt.
                ...
                //------------------------------
                REF.z3_kp1[0] = -0.5f*(OP15_kp1 + OP16_kp1);                                // Compute d^2(xc)/dt^2.
                REF.z3_kp1[1] = -0.5f*(OP17_kp1 + OP18_kp1);                                // Compute d^2(yc)/dt^2.
                // Computing d^2(thc)/dt^2:
                REF.z3_kp1[2] = (OP13_kp1*(OP10_kp1*OP19_kp1 - OP9_kp1*OP20_kp1) - 2.0f*OP21_kp1*OP22_kp1)*OP23_kp1*OP23_kp1;
                // Computing d^2(dc)/dt^2:
                REF.z3_kp1[3] = 0.5f*(OP13_kp1*(OP11_kp1*OP11_kp1 + OP12_kp1*OP12_kp1 + OP22_kp1) - OP22_kp1*OP22_kp1)*OP23_kp1*OP24_kp1;
                switch(consys){
                    case SMC_CSa:
                    REF.z3_kp1[4] = 0.0f;                                                   // Compute d^2(psi_1)/dt^2.
                    REF.z3_kp1[5] = 0.0f;                                                   // Compute d^2(psi_2)/dt^2.
