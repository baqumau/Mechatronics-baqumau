function dataRate = main_04_test_vR2014a()
clc, clear
...
X = zeros(6,6);                                                             % Empty matrix.
iter_stop = (2000/t_sleep)*15.5*60;                                         % Iteration stop.
    ...
    if isempty(data_r)
        ...
        if (isempty(command) == 0) && (command == 9) && (flag_1 == 0)
            ...
            if counter > 0
                ...
                X(3,:) = X(2,:);
                X(2,:) = X(1,:);
                X(1,:) = [x1_t(counter+1),x2_t(counter+1),x3_t(counter+1),x4_t(counter+1),x5_t(counter+1),x6_t(counter+1)];
                for i = 1:length(X(1,:))
                ...
                for i = 1:length(X(:,1))
                    X(i,:) = [x1_t(counter+1),x2_t(counter+1),x3_t(counter+1),x4_t(counter+1),x5_t(counter+1),x6_t(counter+1)];
                end
            end
