...
par.dny_2 = 22;
%% Clutch:
ck = max(abs([par.q0_r1;par.q0_r2] - [par.q0r_r1;par.q0r_r2]))*20/7000 + 6; % Time for clutch.
...
%% Control system gains for multiple simulations:
...
for i = 1:j
    ...
    par.alpha_1 = diag([K1(1,2)+K1(1,2)*(.2 - abs(rand(1)/2.5)) K1(1,2)+K1(1,2)*(.2 - abs(rand(1)/2.5)) K2(1,2)+K2(1,2)*(.2 - abs(rand(1)/2.5)) K1(1,2)+K1(1,2)*(.2 - abs(rand(1)/2.5)) K2(1,2)+K1(1,2)*(.2 - abs(rand(1)/2.5)) K2(1,2)+K2(1,2)*(.2 - abs(rand(1)/2.5))]);
    par.alpha_2 = diag([K1(1,3)+K1(1,3)*(.2 - abs(rand(1)/2.5)) K1(1,3)+K1(1,3)*(.2 - abs(rand(1)/2.5)) K2(1,3)+K2(1,3)*(.2 - abs(rand(1)/2.5)) K1(1,3)+K1(1,3)*(.2 - abs(rand(1)/2.5)) K2(1,3)+K1(1,3)*(.2 - abs(rand(1)/2.5)) K2(1,3)+K2(1,3)*(.2 - abs(rand(1)/2.5))]);
    par.alpha_3 = diag([K1(1,4)+K1(1,4)*(.2 - abs(rand(1)/2.5)) K1(1,4)+K1(1,4)*(.2 - abs(rand(1)/2.5)) K2(1,4)+K2(1,4)*(.2 - abs(rand(1)/2.5)) K1(1,4)+K1(1,4)*(.2 - abs(rand(1)/2.5)) K2(1,4)+K1(1,4)*(.2 - abs(rand(1)/2.5)) K2(1,4)+K2(1,4)*(.2 - abs(rand(1)/2.5))]);
    par.err = .42+.42*(.2 - abs(rand(1)/2.5));
    % Pre-computing constans in function of par.alpha_[1,2,3]:
    par.HGOcons11 = par.alpha_1/par.err;                                    % Precompute constant 11 of HGO 2 (par.alpha_1/par.err).
    par.HGOcons12 = par.alpha_2/par.err^2;                                  % Precompute constant 12 of HGO 2 (par.alpha_2/par.err^2).
    par.HGOcons13 = par.alpha_3/par.err^3;                                  % Precompute constant 13 of HGO 2 (par.alpha_3/par.err^3).
    % Aleatorial tuning parameters of Sliding mode control:
    % Constant for saturate sigma surface:
    par.S_b = [180 180 4*pi/3 150 pi pi]';                                  % Constant for saturate sigma surface.
    par.Eta = [1.84+1.84*(.2-abs(rand(1)/2.5));...
               1.84+1.84*(.2-abs(rand(1)/2.5));...
               1.96+1.96*(.2-abs(rand(1)/2.5));...
               1.84+1.84*(.2-abs(rand(1)/2.5));...
               6.14+6.14*(.2-abs(rand(1)/2.5));...
               6.14+6.14*(.2-abs(rand(1)/2.5))];                            % Constant for bounding the sliding surface dynamics.
    ...
    % Coefficients for sliding surface:
    par.Lambda = diag([1.8432+1.8432*(.2-abs(rand(1)/2.5))...
                       1.8432+1.8432*(.2-abs(rand(1)/2.5))...
                       1.8432+1.8432*(.2-abs(rand(1)/2.5))...
                       1.8432+1.8432*(.2-abs(rand(1)/2.5))...
                       2.7241+2.7241*(.2-abs(rand(1)/2.5))...
                       2.7241+2.7241*(.2-abs(rand(1)/2.5))]);
    % Executing iterative simulation:
    clear functions;
    % Loading nonlinear Adams model:
    twoRobotsPlant_1_v2015
    % Loading control system 1 (SMC-CS):
    conSys_2robots_2_vR2014a
    % Starting simulation:
    SimOut = sim('conSys_2robots_2_vR2014a','SrcWorkspace','Current','StopTime', '120');
    % Storing data:
    tracking_errors = SimOut.get('tracking_errors');
    e1.time = tracking_errors.time;
    e1.signals.values(:,i) = tracking_errors.signals.values(:,1);           % Storing tracking error e_1(t);
    e2.time = tracking_errors.time;
    e2.signals.values(:,i) = tracking_errors.signals.values(:,2);           % Storing tracking error e_2(t);
    e3.time = tracking_errors.time;
    e3.signals.values(:,i) = tracking_errors.signals.values(:,3);           % Storing tracking error e_3(t);
    e4.time = tracking_errors.time;
    e4.signals.values(:,i) = tracking_errors.signals.values(:,4);           % Storing tracking error e_4(t);
    e5.time = tracking_errors.time;
    e5.signals.values(:,i) = tracking_errors.signals.values(:,5);           % Storing tracking error e_5(t);
    e6.time = tracking_errors.time;
    e6.signals.values(:,i) = tracking_errors.signals.values(:,6);           % Storing tracking error e_6(t);
end